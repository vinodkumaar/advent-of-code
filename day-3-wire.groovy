...
intersections = { wirePoints1, wirePoints2 ->
                    return wirePoints2.intersect( wirePoints1)
                }

toManhattanDistance = { wirePoints, startingPoint -> 
                    return wirePoints.collect {wirePoint -> Math.abs(wirePoint.x - startingPoint.x) + Math.abs(wirePoint.y - startingPoint.y)}

                }

R = {startingPoint, distance ->
        wirePoints = []        
        (0..distance.toInteger()).each { point -> 
                wirePoint = [:]
                wirePoint.x = startingPoint.x + point
                wirePoint.y = startingPoint.y 
...
U = {startingPoint, distance ->
        ...
        (0..distance.toInteger()).each { point -> 
                wirePoint = [:]
                wirePoint.x = startingPoint.x 
                wirePoint.y = startingPoint.y + point
                wirePoints.add(wirePoint)      
        }      
        return wirePoints
     }
...
//tests
assert drawWire(['x':1,'y':1], "U3") == [['x':1,'y':1],['x':1,'y':2],['x':1,'y':3],['x':1,'y':4]]
assert drawWire(['x':1,'y':4], "R3") == [['x':1,'y':4],['x':2,'y':4],['x':3,'y':4],['x':4,'y':4]]
assert drawWire(['x':4,'y':4], "D3") == [['x':4,'y':4],['x':4,'y':3],['x':4,'y':2],['x':4,'y':1]]
assert drawWire(['x':4,'y':1], "L3") == [['x':4,'y':1],['x':3,'y':1],['x':2,'y':1],['x':1,'y':1]]
assert intersections([['x':1,'y':1],['x':1,'y':2],['x':1,'y':3],['x':1,'y':4]],
                        [['x':4,'y':1],['x':3,'y':1],['x':2,'y':1],['x':1,'y':1]]) == [['x':1,'y':1]]
assert toManhattanDistance([['x':4,'y':1],['x':3,'y':1],['x':2,'y':1],['x':1,'y':1]], ['x':1,'y':1]) == [3,2,1,0]      
...
manhattanDistances.removeAll {it == 0}
